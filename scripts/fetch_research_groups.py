...
* ``Source``: ``manual``, ``openalex`` or ``unresolved``.
...
def main() -> int:
    ...
    for link in links:
        ...
        if work:
            ...
            rows.append({"Link": link, **info, "Source": "openalex"})
            print(f"ok   {info['Research Group'] or '?':30.30} {info['Org Type']:8} {link}")
        else:
            unresolved += 1
    ...
    print(f"\n{len(rows)} papers, {lookups} lookups, {unresolved} unresolved -> {OUT}")
